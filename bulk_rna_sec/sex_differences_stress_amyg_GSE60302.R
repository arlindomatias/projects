...
gsms <- "XXXXXXXXXX00000XXXXXXXXXX11111"
...
gs <- factor(sml)
groups <- make.names(c("Vulnerable Female","Vulnerable Male"))
levels(gs) <- groups
gset$group <- gs
...
tt <- total[!is.na(tt$Gene.symbol) & tt$Gene.symbol != "", ]
...
colData <- subset(metadados, select=c("gender:ch1"))
...
ego_up <- enrichGO(gene = up_genes$Gene.symbol,
                   #universe = tt$SYMBOL,
                   OrgDb         = org.Rn.eg.db,   
                   keyType       = "SYMBOL",
                   ont           = "MF",
...
ego_down <- enrichGO(gene = down_genes$Gene.symbol,
                     #universe = tt$SYMBOL,
                     OrgDb         = org.Rn.eg.db,   
                     keyType       = "SYMBOL",
                     ont           = "MF",
                     ...
                     pvalueCutoff  = 0.05)
# Gráficos
png("egoupcc.png", width = 2000, height = 2000, res = 300)
dotplot(ego_up,
...
dev.off()
png("ego_up_barplot.png", width = 2000, height = 2000, res = 300)
...
entrez_single <- entrez_counts %>%
  group_by(Gene.ID) %>%
  summarise(
    Gene.symbol = dplyr::first(Gene.symbol),
    across(where(is.numeric), mean)
  )
entrez_single <- merge(entrez_single, tt, by = c("Gene.ID", "Gene.symbol"))
