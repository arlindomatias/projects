...
gset <- getGEO("GSE6858", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
par(mar=c(7,4,2,1))
title <- paste ("GSE6858", "/", annotation(gset), sep ="")
boxplot(ex[,ord], boxwex=0.6, notch=T, main=title, outline=FALSE, las=2, col=gs[ord])
legend("topleft", groups, fill=palette(), bty="n")

# expression value distribution
par(mar=c(4,4,2,1))
title <- paste ("GSE6858", "/", annotation(gset), " value distribution", sep ="")
